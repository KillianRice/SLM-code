%Binary phase grating with translational symmetry producing odd number of
%spots: 13
clear all;
close all;
...
x_n = [0.1289 0.3233 0.5862 0.6139 0.7290 0.7919 0.9074]; % transitions
x_n_T = round(x_n*T) + 1;                                   % rounded off to integer
phi_n = [0,pi,0,pi,0,pi,0,pi];
n = 1:T;
...
Phi = uint8(Phi);
imwrite(Phi,'Dammann_2D_phase_13.bmp','bmp');
